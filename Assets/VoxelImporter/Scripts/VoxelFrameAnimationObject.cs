...
namespace VoxelImporter {
	...
	public class VoxelFrameAnimationObject : VoxelBase {
		...
		void LateUpdate ()
		{
			...
			if (rendererMaterial0 != playMaterial0 ||
				...
				if (playMaterial0 != null)
					count++;
				if (playMaterial1 != null)
					count++;
				if (playMaterial2 != null)
					count++;
				if (playMaterial3 != null)
					count++;
				if (playMaterial4 != null)
					count++;
				if (playMaterial5 != null)
					count++;
				if (playMaterial6 != null)
					count++;
				if (playMaterial7 != null)
					count++;
				//
				...
				int i = 0;
				if (playMaterial0 != null)
					rendererMaterials [i++] = playMaterial0;
				if (playMaterial1 != null)
					rendererMaterials [i++] = playMaterial1;
				if (playMaterial2 != null)
					rendererMaterials [i++] = playMaterial2;
				if (playMaterial3 != null)
					rendererMaterials [i++] = playMaterial3;
				if (playMaterial4 != null)
					rendererMaterials [i++] = playMaterial4;
				if (playMaterial5 != null)
					rendererMaterials [i++] = playMaterial5;
				if (playMaterial6 != null)
					rendererMaterials [i++] = playMaterial6;
				if (playMaterial7 != null)
					rendererMaterials [i++] = playMaterial7;
				//
					...
					if (frame.name == frameName) {
						...
						mesh = frame.mesh;
						{
							for (int i = 0; i < frame.materialIndexes.Count; i++) {
								...
								switch (i) {
								case 0:
									playMaterial0 = materials [index];
									break;
								case 1:
									playMaterial1 = materials [index];
									break;
								case 2:
									playMaterial2 = materials [index];
									break;
								case 3:
									playMaterial3 = materials [index];
									break;
								case 4:
									playMaterial4 = materials [index];
									break;
								case 5:
									playMaterial5 = materials [index];
									break;
								case 6:
									playMaterial6 = materials [index];
									break;
								case 7:
									playMaterial7 = materials [index];
									break;
								default:
		...
		public void Edit_SetPlayMaterials (Material[] mats)
		{
			...
			if (mats.Length > 0)
				playMaterial0 = mats [0];
			if (mats.Length > 1)
				playMaterial1 = mats [1];
			if (mats.Length > 2)
				playMaterial2 = mats [2];
			if (mats.Length > 3)
				playMaterial3 = mats [3];
			if (mats.Length > 4)
				playMaterial4 = mats [4];
			if (mats.Length > 5)
				playMaterial5 = mats [5];
			if (mats.Length > 6)
				playMaterial6 = mats [6];
			if (mats.Length > 7)
				playMaterial7 = mats [7];
			if (mats.Length > 8) {
